...
export const SetItem: React.FC<Props> = ({ data, onUpdate, onDelete }) => {
  ...
  const handleInputChange = (field: 'weight' | 'reps', value: string) => {
    ...
    onUpdate({ ...data, [field]: updatedValue === '' ? null : updatedValue });
  };

  const handleRefreshClick = () => {
    // Заполняем данные из props (data), преобразуя null/undefined в пустую строку
    ...
    <div className="w-full bg-bgBase rounded-2xl text-primary mb-5 overflow-hidden">
      ...
      <div className="flex items-center justify-between space-x-4 max-w-[430px] mx-auto p-5">
        ...
        <div className="flex items-center justify-end w-[10%]">
          <Button variant="icons" size="icons" onClick={handleRefreshClick} className="bg-none">
            {isAutoFilled ? (
